...
def log_memory(message):
    ...
    logger.info(f"{message}: current_memory={current / 1024**2:.2f}MB, peak_memory={peak / 1024**2:.2f}MB")

def extract_search_id(url: str):
    logger.debug(f"Entering extract_search_id with url={url}")
    match = re.search(r"/search/(\d+)/", url)
    result = match.group(1) if match else None
    logger.debug(f"Exiting extract_search_id with result={result}")
    return result

def build_search_url(search_id, query, newer_than, older_than, page=None, older_than_ts=None, title_only=0):
    logger.debug(f"Entering build_search_url with search_id={search_id}, query={query}, title_only={title_only}")
    ...
    if page:
        params["page"] = page
    result = f"{base_url}?{urlencode(params)}"
    logger.debug(f"Exiting build_search_url with result={result}")
    return result

def find_view_older_link(html_str: str, title_only: int = 0):
    logger.debug(f"Entering find_view_older_link with title_only={title_only}")
    tree = HTMLParser(html_str)
    link_node = tree.css_first("div.block-footer a")
    if not link_node or not link_node.attributes.get("href"):
        logger.debug("Exiting find_view_older_link: No link found")
        return None
    href = link_node.attributes["href"]
    match = re.search(r"/search/(\d+)/older.*?before=(\d+).*?[&?]q=([^&]+)", href)
    if not match:
        logger.debug("Exiting find_view_older_link: No match in href")
        return None
    ...
    logger.debug(f"Exiting find_view_older_link with result={result}")
    return result

def get_total_pages(html_str: str):
    logger.debug("Entering get_total_pages")
    tree = HTMLParser(html_str)
    nav = tree.css_first("ul.pageNav-main")
    if not nav:
        logger.debug("Exiting get_total_pages: No nav, returning 1")
        return 1
    ...
    logger.debug(f"Exiting get_total_pages with result={result}")
    return result

def extract_threads(html_str: str):
    logger.debug("Entering extract_threads")
    tree = HTMLParser(html_str)
    threads = []
    ...
    logger.debug(f"Exiting extract_threads with {len(threads)} threads")
    return threads
...
async def fetch_page(client, url: str):
    logger.debug(f"Entering fetch_page with url={url}")
    log_memory("Before fetch_page request")
    try:
        r = await client.get(url, follow_redirects=True, timeout=TIMEOUT)
        result = {"ok": r.status_code == 200, "html": r.text, "final_url": str(r.url)}
    except Exception as e:
        logger.error(f"Error in fetch_page: {e}")
        result = {"ok": False, "html": "", "final_url": url}
    log_memory("After fetch_page request")
    logger.debug(f"Exiting fetch_page with ok={result['ok']}")
    return result
...
async def process_batch(client, batch_num, start_url, query, title_only):
    ...
    log_memory(f"Start of process_batch #{batch_num}")
    resp = await fetch_page(client, start_url)
    if not resp["ok"]:
        logger.error(f"Failed batch start URL in process_batch #{batch_num}")
        log_memory(f"End of process_batch #{batch_num} (failed)")
        return None, None
    ...
    log_memory(f"End of process_batch #{batch_num}")
    gc.collect()
    return batch_data, next_batch_url

async def collect_threads(query, title_only, threads_dir):
    logger.debug(f"Entering collect_threads with query={query}, title_only={title_only}")
    log_memory("Start of collect_threads")
    ...
    async with httpx.AsyncClient() as client:
        while current_url:
            ...
            logger.info(f"Batch #{batch_num}: {total} threads saved to {file_name}")
            ...
            batch_num += 1
    log_memory("End of collect_threads")
    gc.collect()
    logger.debug("Exiting collect_threads")

# ───────────────────────────────
# 📺 ARTICLE COLLECTOR
# ───────────────────────────────
async def make_request(client: httpx.AsyncClient, url: str, retries=MAX_RETRIES) -> str:
    logger.debug(f"Entering make_request with url={url}")
    log_memory("Before make_request")
    for attempt in range(1, retries + 1):
        try:
            resp = await client.get(url, follow_redirects=True, timeout=TIMEOUT)
            resp.raise_for_status()
            result = resp.text
            log_memory("After successful make_request")
            logger.debug("Exiting make_request successfully")
            return result
        except Exception as e:
            logger.warning(f"Attempt {attempt} failed in make_request: {e}")
            if attempt < retries:
                await asyncio.sleep(RETRY_DELAY)
            else:
                log_memory("After failed make_request")
                logger.debug("Exiting make_request with empty string")
                return ""

def article_matches_patterns(article, patterns):
    logger.debug("Entering article_matches_patterns")
    try:
    ...
    for pat in patterns:
        if pat.search(article_text):
            logger.debug("Exiting article_matches_patterns: Match found in text")
            return True
        ...
        try:
            ...
            for pat in patterns:
                if pat.search(el_text):
                    logger.debug("Exiting article_matches_patterns: Match found in element")
                    return True
        except Exception:
            pass
    logger.debug("Exiting article_matches_patterns: No match")
    return False

async def process_thread(client: httpx.AsyncClient, post_url, patterns, semaphore):
    logger.debug(f"Entering process_thread with post_url={post_url}")
    log_memory("Start of process_thread")
    async with semaphore:
        html_str = await make_request(client, post_url)
        if not html_str:
            logger.warning(f"No HTML in process_thread for {post_url}")
            log_memory("End of process_thread (no HTML)")
            return []
        ...
        log_memory("End of process_thread")
        gc.collect()
        logger.debug(f"Exiting process_thread with {len(result)} matched articles")
        return result

async def process_threads_concurrent(thread_urls, patterns):
    logger.debug(f"Entering process_threads_concurrent with {len(thread_urls)} urls")
    log_memory("Start of process_threads_concurrent")
    semaphore = asyncio.Semaphore(MAX_CONCURRENT_WORKERS)
    async with httpx.AsyncClient() as client:
        tasks = [process_thread(client, url, patterns, semaphore) for url in thread_urls]
        results = await asyncio.gather(*tasks)
    result = [item for sublist in results for item in sublist]
    log_memory("End of process_threads_concurrent")
    gc.collect()
    logger.debug(f"Exiting process_threads_concurrent with {len(result)} articles")
    return result
...
def extract_media_from_html(raw_html: str):
    logger.debug("Entering extract_media_from_html")
    if not raw_html:
        logger.debug("Exiting extract_media_from_html: No HTML")
        return []
    ...
    result = list(dict.fromkeys(media_urls))
    logger.debug(f"Exiting extract_media_from_html with {len(result)} media")
    return result

def filter_media(media_list, seen_global):
    logger.debug("Entering filter_media")
    filtered = []
    seen_local = set()
    for url in media_list:
        ...
        if url not in seen_local and url not in seen_global:
            ...
            seen_global.add(url)
            filtered.append(url)
    logger.debug(f"Exiting filter_media with {len(filtered)} filtered media")
    return filtered

async def process_articles_batch(batch_num, articles_file, media_dir):
    logger.info(f"Batch #{batch_num}: Extracting Media")
    log_memory(f"Start of process_articles_batch #{batch_num}")
    ...
    try:
        ...
        log_memory(f"End of process_articles_batch #{batch_num} (error)")
        return
    ...
    for idx, entry in enumerate(articles):
        if idx % 10 == 0:  # Log every 10 articles for progress
            logger.info(f"Processing article {idx}/{total}")
            log_memory(f"During process_articles_batch #{batch_num} at {idx}/{total}")
    ...
    logger.info(f"Media extracted: {len(all_results)} posts saved to {media_output}")
    logger.info(f"No media in {len(no_media_posts)} posts")
    log_memory(f"End of process_articles_batch #{batch_num}")
    gc.collect()
...
def create_html(media_by_date_per_username, usernames, start_year, end_year):
    logger.debug(f"Entering create_html with {len(usernames)} usernames")
    log_memory("Start of create_html")
    ...
    if total_items == 0:
        ...
        log_memory("End of create_html (no items)")
        return None
    ...
    if estimated_size > MAX_FILE_SIZE_MB:
        logger.error(f"Estimated HTML size {estimated_size:.2f} MB exceeds limit of {MAX_FILE_SIZE_MB} MB")
        log_memory("End of create_html (size exceed)")
        return None
    ...
    try:
        ...
        log_memory("End of create_html (serialization fail)")
        return None
...
<head>
  <meta charset="UTF-8">
  <title>{html.escape(usernames_str)} - Media Gallery</title>
  <style>
...
<body>
  ...
  <script>
    ...
    log_memory("End of create_html")
    logger.debug("Exiting create_html")
    return html_content
...
async def upload_file(client, host, data):
    logger.debug(f"Entering upload_file to {host['name']}")
    log_memory("Start of upload_file")
    buf = BytesIO(data)
        ...
        if r.status_code in (200,201):
            if host["name"]=="HTML Hosting":
                ...
                if j.get("success") and j.get("url"):
                    result = (host["name"], j["url"])
                else:
                    result = (host["name"], f"Error: {j.get('error','Unknown')}")
            else:
                t = r.text.strip()
                if t.startswith("https://"):
                    ...
                    result = (host["name"], f"Invalid response: {t[:100]}")
        else:
            result = (host["name"], f"HTTP {r.status_code}")
    except Exception as e:
        result = (host["name"], f"Exception: {e}")
    log_memory("End of upload_file")
    gc.collect()
    logger.debug(f"Exiting upload_file with result={result}")
    return result
...
async def process_user(user, title_only, user_idx, total_users, progress_msg, last_edit):
    ...
    log_memory(f"Start of process_user {user}")
    ...
    log_memory(f"After collect_threads for {user}")
    ...
    for idx, batch_file in enumerate(batch_files, 1):
        ...
        all_articles = await process_threads_concurrent(all_threads, PATTERNS)
        ...
        log_memory(f"After process_articles_batch {idx} for {user}")
    ...
    log_memory(f"After deduplication for {user}")
    ...
    try:
        ...
        logger.info(f"Cleaned up directories for {user}")
    except Exception as e:
        logger.error(f"Cleanup failed for {user}: {e}")
   
    log_memory(f"End of process_user {user}")
    gc.collect()
    return user_data
...
async def handle_message(client: Client, message: Message):
    ...
    if not usernames:
        ...
        log_memory("End of handle_message (no usernames)")
        return
    ...
    for user_idx, user in enumerate(usernames):
        ...
        log_memory(f"After extending all_data for user {user}")
    ...
    log_memory("After building media_by_date_per_username")
    ...
    if html_content:
        with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
            f.write(html_content)
        log_memory("After writing final HTML")
    ...
    log_memory("End of handle_message")
    gc.collect()
